#!/usr/bin/env python
"""
Text + Flow CoT LoRA Training Script for NORA (Phase C).

Train Text + Flow CoT LoRA models (LoRA_TF) on LIBERO subsets with reasoning dropout.
Supports specifying LIBERO subset, ECoT annotations, flow tokens, and reasoning dropout probability.

Requirements covered:
- 7.8: Train with flow_tokens loss
- 7.9: Train separate LoRA_TF weights for each LIBERO subset
- 7.10: Save each LoRA_TF weights separately from base model with subset identifier

Usage:
    # Train on LIBERO-Object subset
    python scripts/train_text_flow_cot.py --subset object --data_dir ./data \
        --ecot_annotations ./data/ecot_annotations/object.json \
        --flow_tokens ./data/flow_tokens/object.json
    
    # Train with custom config
    python scripts/train_text_flow_cot.py --config configs/text_flow_cot.yaml \
        --ecot_annotations ./data/ecot_annotations/object.json \
        --flow_tokens ./data/flow_tokens/object.json
    
    # Train with custom reasoning dropout probability
    python scripts/train_text_flow_cot.py --subset object --reasoning_dropout_prob 0.3
    
    # Resume from checkpoint
    python scripts/train_text_flow_cot.py --subset object --resume outputs/text_flow_cot/object/step_20000
"""
import argparse
import logging
import sys
...
from training.lora_trainer import TextFlowCoTTrainer


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train Text + Flow CoT LoRA model on LIBERO subset (Phase C)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to preprocessed flow tokens JSON file",
    )
    ...
    # Reasoning Dropout arguments (Req 7.5, 7.6)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight for CoT + Flow auxiliary loss (lambda)",
    )
...
def main():
    ...
    # Validate flow tokens path
    flow_path = Path(args.flow_tokens)
    if not flow_path.exists():
        logger.error(f"Flow tokens file not found: {flow_path}")
        sys.exit(1)
    ...
    if args.config:
        ...
        # Update reasoning dropout config
    ...
    # Log configuration
    logger.info("=" * 60)
    logger.info("Text + Flow CoT LoRA Training Configuration (Phase C)")
    logger.info("=" * 60)
    ...
    logger.info(f"  CoT + Flow loss weight (lambda): {config.reasoning_dropout.cot_loss_weight}")
    logger.info(f"  ECoT annotations: {args.ecot_annotations}")
    logger.info(f"  Flow tokens: {args.flow_tokens}")
    ...
    trainer = TextFlowCoTTrainer(
        ...
        flow_tokens_path=str(args.flow_tokens),
    ...
    # Train
    logger.info("Starting Text + Flow CoT training...")
